function [Ig, Id] = SMU_idvg_singlesweep(SDA, gate, drain, holdtime, delaytime, s_delay, ...
    ...
    writeread(SDA, 'ERR? 1');
    writeread(SDA, 'NUB?');
    data = read(SDA, 16*5*nop1+1,'char');
    ...
    Id = [];
    for i=1:nop1
        Ig(i) = str2num(data(1, 20+16*5*(i-1):31+16*5*(i-1)));
        Id(i) = str2num(data(1, 52+16*5*(i-1):63+16*5*(i-1)));
    end
