function [] = wv_pulse_bipolar(wavename, amp, period, rise_fall_time, datasize)
%% waveform & measurement event 
% waveform of bipolar pulse
...
% datasize: number of measure points in a period
    ...
    stage_time = period/2 - rise_fall_time*2;
...
%% create the pattern
    ...
    calllib('wgfmu', 'WGFMU_addVector', wavename1, rise_fall_time*2, -amp);
    if (stage_time>0)
        calllib('wgfmu', 'WGFMU_addVector', wavename1, stage_time, -amp);
    end
    calllib('wgfmu', 'WGFMU_addVector', wavename1, rise_fall_time,  0);
