function [] = wv_pundback_fet(wavename, amp, period, offset, vd, datasize)
...
%     calllib('wgfmu', 'WGFMU_addVector', wavename1, period/2,  offset-amp);
    calllib('wgfmu', 'WGFMU_addVector', wavename2, period/2,   offset+amp);
    calllib('wgfmu', 'WGFMU_addVector', wavename2, period/2,  offset);
    calllib('wgfmu', 'WGFMU_addVector', wavename2, period/2,  offset+amp);
    calllib('wgfmu', 'WGFMU_addVector', wavename2, period,  offset-amp);
    calllib('wgfmu', 'WGFMU_addVector', wavename2, period/2,  offset);
    calllib('wgfmu', 'WGFMU_addVector', wavename2, period/2,  offset-amp);
    calllib('wgfmu', 'WGFMU_addVector', wavename2, period/2,  offset);

    % waveform for channel2
    calllib('wgfmu', 'WGFMU_createPattern', wavename1, vd);
    calllib('wgfmu', 'WGFMU_addVector', wavename1, period*4, vd);
